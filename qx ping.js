...
const storeKey = "pingme_multi_accounts_qx_v1";
const cursorKey = "pingme_batch_cursor_qx_v1";

const SECRET = "0fOiukQq7jXZV2GRi9LGlO";

// ===== 防风控参数 =====
const BATCH_SIZE = 3;          // 每次最多跑几个账号
const MAX_VIDEO = 5;           // 每个账号最多视频次数
const MIN_DELAY = 8000;        // 最小延迟 8 秒
const MAX_DELAY = 15000;       // 最大延迟 15 秒
const STOP_ON_CAPTCHA = true;  // 遇验证码立即停止该账号
// =====================
...
function writeAccounts(accounts) {
  return $prefs.setValueForKey(JSON.stringify(accounts), storeKey);
}
...
function getBatchCursor() {
  const raw = $prefs.valueForKey(cursorKey);
  const num = Number(raw);
  return Number.isInteger(num) && num >= 0 ? num : 0;
}

function setBatchCursor(cursor) {
  return $prefs.setValueForKey(String(cursor), cursorKey);
}
