// PingMe for Surge
// 作用：抓取 queryBalanceAndBonus 请求参数，并定时执行签到 + 视频奖励
// 用法：
// 1. 在 Surge 配好 [MITM] + [Script]
// 2. 打开一次 PingMe，触发 queryBalanceAndBonus 请求完成抓参
// 3. 之后 cron 会自动执行
...
async function runTask() {
  ...
  try {
    ...
    try {
      ...
      const d = JSON.parse(res.body);
      if (d.retcode === 0) {
        msgs.push(`最新余额：${d.结果.balance} Coins`);
      }
    } catch (e) {}
...
function captureRequest() {
  const capture = {
    url: $请求.url,
    paramsRaw: parseRawQuery($请求.url),
    headers: cloneHeaders($请求.headers || {})
  };

  const ok = $persistentStore.编写(JSON.stringify(capture), ckKey);
  if (ok) {
    notify("✅ 参数抓取成功", "已保存请求头和参数");
    log(`capture = ${JSON.stringify(capture)}`);
  } else {
